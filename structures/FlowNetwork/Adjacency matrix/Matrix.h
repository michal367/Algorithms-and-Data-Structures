...
public:
    ...
    void insertRow(std::size_t row, const T& value = T());
    void insertColumn(std::size_t column, const T& value = T());
...
template<class T>
void Matrix<T>::insertRow(std::size_t row, const T& value){
    if(row >= 0 && row <= m){
        T* new_matrix = new T[(m+1)*n];

        for(int i=0, i2=0; i<m+1;){
            if(i == row){
                for(int j=0; j<n; ++j)
                    new_matrix[i*n + j] = value;
                i++;
            }
            else{
                for(int j=0; j<n; ++j)
...
template<class T>
void Matrix<T>::insertColumn(std::size_t column, const T& value){
    if(column >= 0 && column <= n){
        T* new_matrix = new T[m*(n+1)];

        for(int i=0; i<m; i++)
            for(int j=0, j2=0; j<n+1;){
                if(j == column){
                    new_matrix[i*(n+1) + j] = value;
                    j++;
                }
                else{
